...
target_years <- c(seq(2013,2019,2))
...
slope <- aggregate_custom(slope, temp_for_crs, reproj_method="bilinear")
...
generate_training_data <- function(phenoseries)
{
  ...
  # --- All Label Data ---
  label_df <- rbind(riparian_df, oak_df, chaparral_df, annual_df, agriculture_df, soil_df, water_df, urban_df)#, euccup_df)
  label_df <- label_df %>% 
...
predict_raster <- function(phenoseries, year, modFit_rf)
{
...
# Actually apply the above methods across the timeseries
for(year in target_years)
{
  print(paste("Starting to process ",year))
  # Load Data
  phenoseries <- stack(load_phenoseries(year),slope,dem,channel_dist)
  # Sample Training Data
  training_data <- generate_training_data(phenoseries)
  train <- training_data[[1]]
  valid <- training_data[[2]]
  leftover <- training_data[[4]]
