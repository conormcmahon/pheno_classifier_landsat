...
generate_pheno_raster <- function(type)
{
  for(current_year in target_years)
  {
    ...
    output_filename <- paste(directory,"/pheno_",type,"_",current_year,"_annual.tif",sep="")
    output_count_filename <- paste(directory,"/pheno_",type,"_",current_year,"_scene_count_annual.tif",sep="")
    print(paste("Starting work on ", output_filename, ", which is ",num_cols,"x",num_rows,"in size.", sep=""))
    
    # Iterate over all pixels in scene, fitting phenology curve for each individually 
    ...
    for(northing_i in unique(northings))
    {
      ...
      for(pixel in data_i_list)
      {
        ...
        new_series <- approx(pixel$doy,pixel$value,xout=((1:12)*(365/12)))
